...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'Use `artifact://<artifactId>` as the stable reference for an existing Artifact. Desktop can copy a reusable Markdown reference from the Artifact detail page, and Gateway can call `desktop.artifacts.describe` to retrieve the same reference with title, type, value summary, preview card, source, repository output / preview paths, and file or URL clues.',
    '',
    ...
    '',
    'Repository `outputs/index.md` is a skim-readable Artifact directory. Each Artifact entry should expose the artifact URI, source, updated time, preview, format, summary, content extraction status, preview card, reuse kind, and tags when available, while detailed audit facts stay in the per-Artifact output markdown.',
    '',
