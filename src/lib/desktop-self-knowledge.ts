...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    'Workbench Snapshot includes `runs/assets/index.md` in Repository runs. Dashboard reads Repository asset run entries as recent and weekly output clues, skips entries already represented by known Artifact ids or Repository output paths, and surfaces `asset-run:review-pending` confirmations when the run index says `review: pending`. These are observation and routing clues only; Dashboard does not write the review, execute assets, grant permission, update matter status, preserve outputs, or update knowledge automatically.',
    '',
