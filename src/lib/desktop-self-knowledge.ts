...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'For imported text/code/HTML file Artifacts, Gateway can call `desktop.artifacts.content.extract` with `artifactId` and optional `repoPath`. This reads only imported text/code Artifact copies already stored under Desktop Artifact storage, records `contentExtract` facts (bytes read, text length, truncation, and snippet), and can mirror the updated Repository output. It does not read arbitrary local paths, parse Office/PDF/media files, execute commands, or grant permissions.',
    '',
