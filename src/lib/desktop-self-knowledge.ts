...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'When a Repository-local executable asset (`tool`, `script`, or `workflow`) has been approved, run, or completed by an external runner, Gateway can call `desktop.repository.assets.execution.record` with `repoPath`, `assetId` or Repository-relative `path`, `status`, optional `runner`, `command`, `resultSummary`, `outputArtifactId`, `repositoryOutputPath`, `workItemPath`, and `executedAt`. Desktop writes a Markdown execution record under `runs/assets/`, maintains `runs/assets/index.md` as the skim-readable run index, refreshes `outputs/assets/index.md` with latest execution, lastRun, and terminal review clues, and keeps the hard boundary `recordOnly, desktopExecutes=false, grantsPermission=false`. This command records repository asset execution facts only; it does not execute the asset, grant permission, write the review, update a work matter, preserve outputs, or update knowledge automatically.',
    '',
