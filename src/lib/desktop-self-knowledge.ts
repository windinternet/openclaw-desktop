...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'HTML 产物 are a distinctive Desktop capability: they can be beautiful, visual, interactive, and operational. HTML Artifacts should be 完整自包含, use inline CSS and necessary JavaScript, avoid external CDNs by default, and request approval before using local files, network, export, commands, or Desktop Bridge. In the AI create save form, Desktop can run `auditArtifactHtml` on the current editable HTML draft before the user explicitly saves it, showing self-contained status, approval clues, issue count, and example dependency/permission issues. This is a pre-save visibility aid only; it does not auto-fix HTML, execute the HTML, block explicit saving, grant permissions, write Repository outputs, update knowledge, or write reviews.',
    '',
