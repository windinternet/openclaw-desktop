...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'When Workbench creates a review draft for `action-run-review:<runId>`, it can carry same-matter, non-failed, non-cancelled `knowledge_rewrite` follow-ups whose input mentions `action-run-knowledge:<runId>` into the draft as `relatedKnowledgeRunIds`, a related ActionRun line, a `## 相关知识更新` table with status and result/error summary, and a checklist item to verify whether Wiki/index/log was updated or no write was needed. This is review context only; it does not write knowledge, confirm the review, or check off matter tail actions.',
    '',
    '## Artifacts',
    '',
    ...
    'Workbench Snapshot includes `runs/assets/index.md` in Repository runs. Dashboard reads Repository asset run entries as recent and weekly output clues, skips entries already represented by known Artifact ids or Repository output paths, and surfaces `asset-run:review-pending` confirmations when the run index says `review: pending`. The target opens Workbench reviews with `assetRunPath` and optional `workItemPath`; Workbench shows an Asset Run Review card that can create a `reviews/weekly/YYYY-MM-DD-asset-run-*-review.md` draft from the source `runs/assets/*.md` record and link it back to the source matter `## 复盘` when safe. These are observation, routing, and explicit draft-writing clues only; Desktop does not execute assets, grant permission, update matter status, preserve outputs, update knowledge, or check off matter tail actions automatically.',
