...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'After the user or Gateway has review content for the latest terminal executable Artifact run, call `desktop.artifacts.execution.review.write` with `repoPath`, `artifactId`, optional `reviewSummary`, `reuseDecision`, `nextActions`, `workItemPath`, `reviewer`, and `reviewedAt`. This writes a `reviews/weekly/` Markdown review that links the Artifact, latest execution status/result/output clues, optional work item, reuse decision, and next actions. It does not execute the Artifact, grant execution permission, or update the work item; tail-action checkoff and status changes remain separate user-confirmed steps.',
    '',
