...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'Newly imported text/code/HTML file Artifacts automatically record `contentExtract` when Desktop can safely read the imported Artifact storage copy. Gateway can also call `desktop.artifacts.content.extract` with `artifactId` and optional `repoPath` to refresh the same facts later. This reads only imported text/code Artifact copies already stored under Desktop Artifact storage, records `contentExtract` facts (bytes read, text length, truncation, and snippet), and can mirror the updated Repository output. It does not read arbitrary local paths, parse Office/PDF/media files, execute commands, or grant permissions.',
    '',
    ...
    '',
    'Repository `outputs/index.md` is a skim-readable Artifact directory. Each Artifact entry should expose the artifact URI, source, updated time, preview, format, summary, preview plan, content extraction status, preview card, reuse kind, and tags when available, while detailed audit facts stay in the per-Artifact output markdown.',
    '',
