...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'Dashboard first surfaces a work-system summary before Gateway infrastructure details. It groups existing local facts into todayContinue, pendingConfirmations, stuckItems, recentOutputs, and knowledgeUpdates from Sessions, Workbench, Knowledge, ActionRuns, and Artifacts. Workbench tail actions from unfinished `## 收尾动作` checklists count as pending confirmations, are classified into status/output/knowledge/review follow-ups, and route to Workbench, Artifacts, or Knowledge with `tailAction`, `tailActionId`, and `workItemPath` context. Knowledge health issues are computed from Repository `sources/`, `wiki/`, `wiki/index.md`, and Wiki links, including orphan sources, stale index entries, broken knowledge links, unindexed Wiki pages, and Wiki pages without source references; Dashboard surfaces these as knowledgeUpdates that open `/knowledge?section=health`. Knowledge can import pasted text into `sources/imported/` with `source: desktop-paste`, import local text files into `sources/imported/` with `source: desktop-file` and original file metadata, drop local text files into `sources/imported/` through the Knowledge page, clip URLs into `sources/imported/` with `source: desktop-url` and optional excerpts, then refresh the Snapshot, switch to the undigested queue, and open the new source. File import currently covers Markdown/TXT text files through a picker or drag-and-drop; URL clipping does not fetch page bodies yet. Knowledge also exposes an undigested sources queue at `/knowledge?section=digest`; it lists source files that are not indexed and not referenced by Wiki pages, and a Knowledge ActionRun can digest one queued source into reusable Wiki after approval. Dashboard can mark one complete by writing `[x]` back to the source matter. Gateway health remains visible, but it is not the primary answer to "what is my system state?".',
