...
export function buildDesktopSelfKnowledgeSkillContent(): string {
  return [
    ...
    '',
    'Repository `outputs/index.md` is a skim-readable Artifact directory. Each Artifact entry should expose the artifact URI, source, updated time, preview, format, summary, preview card, reuse kind, and tags when available, while detailed audit facts stay in the per-Artifact output markdown.',
    '',
