...
const INSTANCE_AGENT_SWITCH_OPTIONS: InstanceAgentSwitchStrategy[] = ['inherit', 'new-session', 'subagent-session'];
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
          ...
          {desc && (
            <Text type="tertiary" size="small" style={{ display: 'block', marginLeft: 28 }}>
              {desc}
            </Text>
          )}
        </div>
    ...
    >
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: embedded ? '12px 0 0' : '16px 28px 28px', minHeight: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    >
                      <div
                        style={{
                          ...
                          border: isActive ? '2px solid var(--semi-color-text-0)' : '2px solid transparent',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
                    {t('settings.sessionToolCallDisplayDesc')}
                  </Text>
                </div>
                <Switch
                  checked={settings.sessionToolCallDisplay !== 'hidden'}
                  onChange={(checked: boolean) =>
                    updateSettings({ sessionToolCallDisplay: checked ? 'compact' : 'hidden' })
                  }
                />
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
                <div style={{ minWidth: 0 }}>
                  <Text style={{ display: 'block', color: 'var(--semi-color-text-0)' }}>
                    {t('settings.sessionReasoningDisplay')}
                  </Text>
                  <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
                    {t('settings.sessionReasoningDisplayDesc')}
                  </Text>
                </div>
                <Switch
                  checked={settings.sessionReasoningDisplay !== 'hidden'}
                  onChange={(checked: boolean) =>
                    updateSettings({ sessionReasoningDisplay: checked ? 'visible' : 'hidden' })
                  }
                />
              </div>
          ...
          <SectionCard icon="🤖" title={t('settings.agentSwitching')} desc={t('settings.agentSwitchingDesc')}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <Text size="small" style={{ display: 'block', marginBottom: 8, color: 'var(--semi-color-text-1)' }}>
                ...
                <Select
                  ...
                  onChange={(value: unknown) => updateSettings({ agentSwitchStrategy: value as AgentSwitchStrategy })}
                  style={{ width: 280 }}
                >
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 4 }}>
            <Button theme="borderless" type="danger" size="small" onClick={() => resetSettings()}>
              {t('settings.reset')}
            </Button>
          </div>
        </div>
...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'right', fontSize: 13, color: 'var(--semi-color-text-0)' }}>{value}</div>
    </div>
  );
}
