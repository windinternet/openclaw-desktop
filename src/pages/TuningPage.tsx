...
import { IconPlusCircle, IconRefresh, IconCalendar, IconSave, IconEdit } from '@douyinfe/semi-icons';
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
          <Space>
            ...
            <Text weight={600}>{dateStr ? formatDateLabel(dateStr, t) : file.name}</Text>
            {isToday && (
        ...
        {isExpanded && !content && !isLoadingContent && (
          <div style={{ padding: 16, borderTop: '1px solid var(--semi-color-border)' }}>
            <Text type="tertiary">{t('tuning.noContent')}</Text>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <Text strong style={{ fontSize: 15 }}>
            {t('tuning.memoryOverview')}
          </Text>
          <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
            {t('tuning.memoryOverviewDesc')}
          </Text>
        </div>
        <Button icon={<IconRefresh />} onClick={handleRefresh} loading={loading} theme="outline" size="small">
          {t('common.refresh')}
        </Button>
      </div>
      <div
      ...
      >
        <div>
          <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 4 }}>
            {t('tuning.connectionStatus')}
          </Text>
          <Tag color={isConnected ? 'green' : 'red'}>
            {isConnected ? t('tuning.connected') : t('tuning.notConnected')}
          </Tag>
        </div>
        <div>
          <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 4 }}>
            {t('tuning.memoryFiles')}
          </Text>
          <Tag color={memoryFiles.length > 0 ? 'blue' : 'grey'}>
            {t('tuning.nFiles', { count: memoryFiles.length })}
          </Tag>
        </div>
        <div>
          <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 4 }}>
            {t('tuning.todayMemory')}
          </Text>
          <Tag color={todayFiles.length > 0 ? 'green' : 'grey'}>
            {todayFiles.length > 0 ? t('tuning.nEntries', { count: todayFiles.length }) : t('common.none')}
          </Tag>
        </div>
      </div>
      {loading && (
        <div style={{ display: 'flex', justifyContent: 'center', padding: 48 }}>
          <Spin tip={t('tuning.loadingMemory')} />
        </div>
      )}
      ...
      {isConnected && !loading && memoryFiles.length === 0 && (
        <Empty description={t('tuning.noMemory')} style={{ marginTop: 48 }} />
      )}
      {todayFiles.length > 0 && (
      ...
      {yesterdayFiles.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <Text strong style={{ display: 'block', marginBottom: 12 }}>
            {t('tuning.yesterdayMemory')} <Tag size="small">{yesterday}</Tag>
          </Text>
      ...
      {olderFiles.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <Text strong style={{ display: 'block', marginBottom: 12 }}>
            {t('tuning.olderMemory')} <Tag size="small">{t('tuning.nEntries', { count: olderFiles.length })}</Tag>
          </Text>
...
function PersonaTab() {
  ...
  const saveAssistant = useCallback(async () => {
    ...
    try {
      await activeClient.request('agent.identity.set', {
        agentId: agentIdentity?.agentId || 'main',
        name: assistantDraft.trim(),
      });
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      const pf =
        typeof bootstrap?.bootstrapMaxFileChars === 'number'
          ? bootstrap.bootstrapMaxFileChars
          : typeof bootstrap?.maxFileChars === 'number'
            ? bootstrap.maxFileChars
            : null;
      const tot =
        typeof bootstrap?.bootstrapMaxTotalChars === 'number'
          ? bootstrap.bootstrapMaxTotalChars
          : typeof bootstrap?.maxTotalChars === 'number'
            ? bootstrap.maxTotalChars
            : null;
      const fu =
        typeof bootstrap?.bootstrapFollowUp === 'string'
          ? bootstrap.bootstrapFollowUp
          : typeof bootstrap?.followUp === 'string'
            ? bootstrap.followUp
            : null;
      ...
      try {
        await activeClient.request('config.patch', {
          raw: {
            agents: {
              defaults: {
                bootstrapMaxFileChars: preset.perFileChars,
                bootstrapMaxTotalChars: preset.totalChars,
                bootstrapFollowUp: preset.followUp,
              },
            },
          },
        });
        setCurrentPresetId(preset.id);
        setPerFileChars(preset.perFileChars);
        setTotalChars(preset.totalChars);
        setFollowUp(preset.followUp);
        setDraftPerFile(String(preset.perFileChars));
        setDraftTotal(String(preset.totalChars));
        setDraftFollowUp(preset.followUp);
  ...
  const saveCustomValues = useCallback(async () => {
    if (!activeClient || !isConnected) return;
    const pf = parseInt(draftPerFile, 10);
    const tot = parseInt(draftTotal, 10);
    if (!pf || !tot || pf < 1000 || tot < 1000) {
      Toast.warning(t('tuning.invalidValue'));
      return;
    }
    setSaving(true);
    try {
      await activeClient.request('config.patch', {
        raw: {
          agents: {
            defaults: { bootstrapMaxFileChars: pf, bootstrapMaxTotalChars: tot, bootstrapFollowUp: draftFollowUp },
          },
        },
      });
      setPerFileChars(pf);
      ...
      const match = CONTEXT_PRESETS.find(
        (p) => p.perFileChars === pf && p.totalChars === tot && p.followUp === draftFollowUp,
      );
      setCurrentPresetId(match ? match.id : null);
  ...
  const dirty =
    perFileChars != null &&
    (String(perFileChars) !== draftPerFile ||
      String(totalChars) !== draftTotal ||
      (followUp || 'every-turn') !== draftFollowUp);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            <div>
              <Text weight={600}>{t('tuning.aiName')}</Text>
              <Text type="tertiary" size="small">
                {agentIdentity?.agentId ? `Agent: ${agentIdentity.agentId}` : ''}
              </Text>
            </div>
            {editingAssistant ? (
              <Space>
                <Input
                  ...
                  style={{ width: 180 }}
                  placeholder={t('tuning.aiNamePlaceholder')}
                />
                <Button size="small" onClick={saveAssistant} loading={savingAssistant} theme="solid">
                  {t('common.save')}
                </Button>
                <Button size="small" onClick={() => setEditingAssistant(false)} theme="outline">
                  {t('common.cancel')}
                </Button>
              </Space>
            ) : (
              <Space>
                <Text style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 500 }}>{assistantName}</Text>
                <Button
                  icon={<IconEdit />}
                  size="small"
                  onClick={() => {
          ...
          >
            <div>
              <Text weight={600}>{t('tuning.userName')}</Text>
              <Text type="tertiary" size="small">
                {t('tuning.userNameDesc')}
              </Text>
            </div>
            {editingUser ? (
              <Space>
                <Input
                  ...
                  style={{ width: 180 }}
                  placeholder={t('tuning.userNamePlaceholder')}
                />
                <Button size="small" onClick={saveUser} loading={savingUser} theme="solid">
                  {t('common.save')}
                </Button>
                <Button size="small" onClick={() => setEditingUser(false)} theme="outline">
                  {t('common.cancel')}
                </Button>
              </Space>
            ) : (
              <Space>
                <Text style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 500 }}>{displayName}</Text>
                <Button
                  icon={<IconEdit />}
                  size="small"
                  onClick={() => {
                    setUserDraft(displayName);
                    setEditingUser(true);
                  }}
                  theme="borderless"
                />
              ...
              <Space>
                {currentPreset && (
                  <Tag color="green" size="small">
                    {t('tuning.presetMatch', { name: currentPreset.name })}
                  </Tag>
                )}
                <Button
                  icon={<IconSave />}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
              <div>
                <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 6 }}>
                  {t('tuning.bootstrapPerFile')}
                </Text>
                <Input value={draftPerFile} onChange={setDraftPerFile} size="small" placeholder="12000" />
                <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
                  {t('tuning.bootstrapPerFileDesc')}
                </Text>
              </div>
              <div>
                <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 6 }}>
                  {t('tuning.bootstrapTotal')}
                </Text>
                <Input value={draftTotal} onChange={setDraftTotal} size="small" placeholder="60000" />
                <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
                  {t('tuning.bootstrapTotalDesc')}
                </Text>
              </div>
              <div>
                <Text type="tertiary" size="small" style={{ display: 'block', marginBottom: 6 }}>
                  {t('tuning.followUpTurns')}
                </Text>
                <Select
                  value={draftFollowUp}
                  onChange={(v) => setDraftFollowUp(v as string)}
                  size="small"
                  style={{ width: '100%' }}
                >
            ...
            {CONTEXT_PRESETS.map((preset) => {
              ...
              return (
                <button
                  ...
                  style={{
                    textAlign: 'left',
                    border: `1.5px solid ${active ? 'var(--semi-color-success)' : 'var(--semi-color-border)'}`,
                    background: active ? 'var(--semi-color-success-light-default)' : 'var(--semi-color-bg-1)',
                    borderRadius: 10,
                    padding: 16,
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 12, fontSize: 12, color: 'var(--semi-color-text-2)' }}>
                    <span>
                      {preset.perFileChars.toLocaleString()}
                      {t('tuning.perFileUnit')}
                    </span>
                    <span>
                      {preset.totalChars.toLocaleString()}
                      {t('tuning.totalCharsUnit')}
                    </span>
                  ...
                  <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                    {active ? (
                      ...
                      <Tag color="blue" size="small" style={{ opacity: 0 }}>
                        &nbsp;
                      </Tag>
                    )}
    ...
    () => [
      {
        ...
        enabled: true,
        connected: false,
        accounts: [{ id: 'default', name: 'Default', connected: false }],
      },
      {
        id: 'telegram',
        enabled: true,
      ...
      { id: 'imessage', enabled: false, connected: false },
      {
        ...
        enabled: true,
        connected: true,
        accounts: [{ id: 'default', name: '飞书', connected: true, lastSeen: Date.now() - 30000 }],
      },
      { id: 'matrix', enabled: false, connected: false },
      {
        ...
        enabled: true,
        connected: true,
        accounts: [{ id: 'default', name: 'WebChat', connected: true }],
      },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <Text strong style={{ fontSize: 15 }}>
            {t('tuning.channelsTitle')}
          </Text>
          <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
            {t('tuning.channelsDesc')}
          </Text>
        </div>
        <Space>
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundColor: 'var(--semi-color-bg-1)',
                borderLeft: `4px solid ${ch.enabled ? color : 'var(--semi-color-border)'}`,
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <Tag color={connected ? 'green' : ch.enabled ? 'blue' : 'grey'} size="small">
                  {connected ? t('tuning.connected') : ch.enabled ? t('tuning.enabled') : t('tuning.notEnabled')}
                </Tag>
              </div>
              {ch.accounts?.map((acc) => (
                <div key={acc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <Tag color={acc.connected ? 'green' : 'grey'} size="small">
                    {acc.connected ? t('tuning.online') : t('tuning.offline')}
                  </Tag>
                </div>
              ))}
            </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <Button
                ...
                loading={saving === 'dmScope'}
                disabled={!dirtyDmScope || !isConnected}
              >
                {t('common.save')}
              </Button>
            </div>
            <Select
              value={draftDmScope}
              onChange={(v) => setDraftDmScope(v as string)}
              style={{ width: 280 }}
              size="small"
            >
            ...
            <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
              {dmScope !== null
                ? t('tuning.currentValue', {
                    value: DM_SCOPE_OPTIONS.find((o) => o.value === dmScope)?.label || dmScope,
                  })
                : t('tuning.notSetDefault')}
            </Text>
          </div>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <Button
                ...
                loading={saving === 'dmPolicy'}
                disabled={!dirtyDmPolicy || !isConnected}
              >
                {t('common.save')}
              </Button>
            </div>
            <Select
              value={draftDmPolicy}
              onChange={(v) => setDraftDmPolicy(v as string)}
              style={{ width: 240 }}
              size="small"
            >
            ...
            <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
              {dmPolicy !== null
                ? t('tuning.currentValue', {
                    value: DM_POLICY_OPTIONS.find((o) => o.value === dmPolicy)?.label || dmPolicy,
                  })
                : t('tuning.defaultPairing')}
            </Text>
          </div>
        </div>
      </Card>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <Button
                ...
                loading={saving === 'resetTime'}
                disabled={!dirtyResetTime || !isConnected}
              >
                {t('common.save')}
              </Button>
            </div>
            <Select
              ...
              onChange={(v) => setDraftResetTime(v as string)}
              style={{ width: 180 }}
              size="small"
            >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <Text strong style={{ fontSize: 15 }}>
            {t('tuning.modelsTitle')}
          </Text>
          <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
            {t('tuning.modelsTitleDesc')}
          </Text>
        </div>
        <Space>
      ...
      {!isConnected && !loading && defaultModelLoaded && (
        <Empty description={t('tuning.notConnectedGateway')} style={{ marginTop: 48 }} />
      )}
      ...
      {defaultModelLoaded && (
        <>
          ...
          >
            <Text strong style={{ display: 'block', marginBottom: 12 }}>
              {t('tuning.connectedModels')}{' '}
              <Tag color="green" size="small">
                {models.length}
              </Tag>
            </Text>
            {models.length === 0 && !loading && (
              <Empty description={t('tuning.noModelsDesc')} style={{ padding: 20 }} />
            )}
            {models.length > 0 && (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <Button
                ...
                loading={savingModel}
                disabled={!modelsNeedSave || !isConnected}
              >
                {t('common.save')}
              </Button>
            </div>
          ...
          >
            <Text strong style={{ display: 'block', marginBottom: 12 }}>
              {t('tuning.providerDistribution')} <Tag size="small">{providerList.length}</Tag>
            </Text>
            {providerList.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {providerList.map((m) => (
                  <Tag key={m.provider} color="blue" size="small">
                    {m.provider} ({models.filter((x) => x.provider === m.provider).length})
                  </Tag>
                ))}
              </div>
            ) : (
              <Text type="tertiary" size="small">
                {t('tuning.noProviderDetected')}
              </Text>
            )}
          </Card>
        </>
      ...
      <Modal
        ...
        footer={
          <Space>
            <Button onClick={() => setModalVisible(false)} theme="light">
              {t('common.cancel')}
            </Button>
            <Button
              ...
              theme="solid"
              loading={addingModel}
              disabled={!providerId.trim() || !modelId.trim()}
            >
              {t('tuning.add')}
            </Button>
          </Space>
          ...
          <div>
            <Text size="small" style={{ display: 'block', marginBottom: 6, color: 'var(--semi-color-text-1)' }}>
              ...
              <Tag size="small" color="red">
                {t('tuning.providerIdRequired')}
              </Tag>
            </Text>
            <Input value={providerId} onChange={setProviderId} placeholder="my-local" size="small" />
            <Text type="tertiary" size="small" style={{ marginTop: 2 }}>
              {t('tuning.providerIdDesc')}
            </Text>
          </div>
          <div>
            <Text size="small" style={{ display: 'block', marginBottom: 6, color: 'var(--semi-color-text-1)' }}>
...
function GeneralTab() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const config = await activeClient!.request<Record<string, unknown>>('config.get');
        const gwConfig = (config?.parsed as Record<string, unknown> | undefined)?.gateway as
          | Record<string, unknown>
          | undefined;
        const bind = typeof gwConfig?.bind === 'string' ? gwConfig.bind : null;
        if (bind) {
          setBindMode(bind);
          setDraftBindMode(bind);
        }
      } catch {
        /* ignore */
      } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <Text strong style={{ fontSize: 15 }}>
          {t('tuning.generalTitle')}
        </Text>
        <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 4 }}>
          {t('tuning.generalTitleDesc')}
        </Text>
      </div>
      <Card
        style={{
      ...
      >
        ...
        <div>
          <Select
            value={draftBindMode}
            onChange={(v) => setDraftBindMode(v as string)}
            style={{ width: 240 }}
            size="small"
          >
      ...
      >
        <Text strong style={{ display: 'block', marginBottom: 16 }}>
          {t('tuning.desktopBehavior')}
        </Text>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <Text style={{ color: 'var(--semi-color-text-0)' }}>{t('tuning.connectAllOnStartup')}</Text>
            <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 2 }}>
              {t('tuning.desktopOnly')}
            </Text>
          </div>
          <Select
            value={useSettingsStore((s) => (s.settings.connectAllInstancesOnStartup ? 'yes' : 'no'))}
            onChange={(v) => useSettingsStore.getState().updateSettings({ connectAllInstancesOnStartup: v === 'yes' })}
            style={{ width: 80 }}
            size="small"
          >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
          <div>
            ...
            <Text type="tertiary" size="small" style={{ display: 'block', marginTop: 2 }}>
              {t('tuning.openTuningOnStartupDesc')}
            </Text>
          </div>
          <Select
            value={useSettingsStore((s) => (s.settings.openTuningOnStartup ? 'yes' : 'no'))}
            onChange={(v) => useSettingsStore.getState().updateSettings({ openTuningOnStartup: v === 'yes' })}
            style={{ width: 80 }}
            size="small"
          >
            <Select.Option value="yes">{t('common.yes')}</Select.Option>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: embedded ? '12px 0 0' : '0 28px 28px', minHeight: 0 }}>
        <Tabs type="line" activeKey={activeTab} onChange={setActiveTab}>
          <TabPane tab={t('tuning.memory')} itemKey="memory">
            <MemoryTab />
          </TabPane>
          <TabPane tab={t('tuning.persona')} itemKey="persona">
            <PersonaTab />
          </TabPane>
          <TabPane tab={t('tuning.communication')} itemKey="channels">
            <ChannelsTab />
          </TabPane>
          <TabPane tab={t('tuning.models')} itemKey="models">
            <ModelsTab />
          </TabPane>
          <TabPane tab={t('tuning.general')} itemKey="general">
            <GeneralTab />
          </TabPane>
        </Tabs>
      </div>
