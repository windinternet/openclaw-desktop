...
import { DESKTOP_COMPANION_INSTALL_SPEC, DESKTOP_COMPANION_PLUGIN_ID } from '../lib/desktop-companion';
...
export default function MainPage() {
  ...
  const companionInstallDismissedRef = useRef(localStorage.getItem('openclaw-companion-install-dismissed') === '1');
  ...
  useEffect(() => {
    ...
    void useStore
      .getState()
      .detectDesktopCompanionForInstance(currentId)
      .then((info) => {
        if (cancelled) return;
        if (!info) return;
        ...
        if (info.status === 'missing' || info.status === 'disabled') {
          ...
          const handleInstallSession = () => {
            void useStore
              .getState()
              .createDesktopCompanionInstallSessionForInstance(currentId)
              .then((result) => {
          ...
          Toast.warning({
            content: (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <span>
                  OpenClaw Desktop Companion 未安装或未启用。请在 Gateway 主机执行： openclaw plugins install{' '}
                  {DESKTOP_COMPANION_INSTALL_SPEC}，然后执行： openclaw plugins enable {DESKTOP_COMPANION_PLUGIN_ID}
                </span>
  ...
  const handleApproveCompanion = () => {
    ...
    void useStore
      .getState()
      .approveDesktopCompanionForInstance(currentId)
      .then(() => {
  ...
  return (
    <Layout style={{ height: '100vh', boxSizing: 'border-box' }}>
      ...
      >
        <Sidebar onAddInstance={() => setAddModalVisible(true)} onOpenDrawer={() => setDrawerVisible(true)} />
        <div
      ...
      <Modal
        ...
        footer={
          <Space>
            ...
            <Button theme="solid" type="primary" loading={companionApprovalApproving} onClick={handleApproveCompanion}>
              授权并重连
            </Button>
          </Space>
        }
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Text>
            OpenClaw Desktop Companion 需要把当前 Desktop 设备升级为 Gateway node， 才能接收 node.invoke
            并在本地创建或打开产物。
          </Text>
          {companionApprovalRequest && (
            <div style={{ display: 'grid', gridTemplateColumns: '88px minmax(0, 1fr)', gap: '8px 12px' }}>
              ...
              <Text code ellipsis>
                {companionApprovalRequest.requestId}
              </Text>
              ...
              <Space spacing={4} wrap>
                {(companionApprovalRequest.scopes.length > 0
                  ...
                  <Tag key={scope} size="small" color="blue">
                    {scope}
                  </Tag>
                ))}
              </Space>
