...
export class CanvasRenderer {
  ...
  drawBubbles(dt: number): void {
    ...
    for (let i = this.bubbles.length - 1; i >= 0; i--) {
      ...
      this.ctx.strokeText(b.text, b.x + cx, b.y + cy - 40);
      this.ctx.fillText(b.text, b.x + cx, b.y + cy - 40);
      this.ctx.restore();
  ...
  addBubble(text: string, x: number, y: number): void {
    this.bubbles.push({ x, y, text, alpha: 1, vy: -40, life: 3 });
  ...
  addEmojiBubble(emoji: string, x: number, y: number): void {
    this.bubbles.push({ x, y, text: emoji, alpha: 1, vy: -60, life: 2 });
  }
