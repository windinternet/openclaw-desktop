...
describe('deriveOfficeAgents', () => {
  ...
  it('uses active sessions to detect real multi-agent collaboration', () => {
    const agents = [{ ...agent('main', 'idle'), default: true }, agent('researcher', 'idle'), agent('writer', 'idle')];
  ...
  it('treats agents with missing gateway status as idle instead of offline', () => {
    const officeAgents = deriveOfficeAgents([{ id: 'main', name: 'Main Agent', model: 'gpt-4.1' }]);
  ...
  it('moves multiple running agents into a meeting with one presenter', () => {
    ...
    expect(meetingAgents.map((a) => a.behavior)).toEqual(['presenting', 'listening', 'listening']);
