...
休闲区不是状态占位符，而是一个可继续扩展的空间模块。沙发与扶手椅表达 idle Agent 的社交和放松状态，咖啡机、饮水机、茶几、充电垫和小睡点分别承载喝咖啡、补水、短聊、充电、休眠等动作。后续自然语言调整办公室布局时，这些元素需要以可命名、可移动、可增删的布局对象沉淀到本地存储，而不是散落在 Three.js 渲染代码里。
