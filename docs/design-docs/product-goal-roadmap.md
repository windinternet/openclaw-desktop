...
- 当创建 `action-run-review:<runId>` 来源的复盘草稿时，Workbench 会查找同一来源事项下未失败/未取消、且输入包含 `action-run-knowledge:<runId>` 的 `knowledge_rewrite` ActionRun，把这些运行 ID 写入草稿 frontmatter 的 `relatedKnowledgeRunIds`、正文的“相关知识更新 ActionRun”、`## 相关知识更新` 状态/摘要表格和核对清单，帮助用户复盘知识更新是否已写入 Wiki/index/log 或确认无需写入。
