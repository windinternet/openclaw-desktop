...
- Workbench reviews 视图已能接收 Dashboard 的 `assetRunPath`，显示“资产运行复盘”卡片，并基于 `runs/assets/*.md` 创建 `reviews/weekly/YYYY-MM-DD-asset-run-*-review.md` 草稿；Desktop node 也暴露 `desktop.repository.assets.execution.review.write`，让 Gateway 在聊天中可按同一边界写入仓库本地资产运行复盘。该入口只写复盘草稿和可选事项 `## 复盘` 链接，不执行资产、不授予权限、不自动更新事项状态、不沉淀成果、不更新知识库、不勾选事项尾动作。
