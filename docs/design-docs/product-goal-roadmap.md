...
- Dashboard 会把计划元数据中显式声明的 `dependsOn` / `dependencies` / `requires` / `relatedWork` / `依赖事项` / `关联事项` / `前置事项` 解析为跨事项依赖，并作为 `plan:cross-work-risk` 卡住项展示；这只是显式元数据诊断，不从计划正文推断风险，也不自动改写计划。
