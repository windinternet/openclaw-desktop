...
- 仓库本地执行型资产（`tool / script / workflow`）已可通过 `desktop.repository.assets.execution.record` 归档外部 runner 的审批、运行或结果事实：Desktop 会写入 `runs/assets/YYYYMMDD-HHmmss-<assetId>.md`，并刷新 `outputs/assets/index.md` 的 `execution`、`lastRun`、`review` 和 `reviewResult` 线索；该入口只记录事实，不执行资产、不授予权限、不自动写复盘、不更新事项、不沉淀成果、不更新知识库。
