...
- Workbench 活跃计划预览可发起 `plan_execute` ActionRun 执行计划；Desktop 会带入 `planPath`、计划内容、计划元数据中的 `workItemPath` 和来源事项内容，使用 `plan-execute.md` / `buildPlanExecutePrompt` 要求 Agent 推进计划。涉及副作用时仍需 `approval_required`；终态后复用 ActionRun 仓库摘要、事项执行记录和收尾动作机制。该入口不自动沉淀成果、不更新知识库或写复盘。
