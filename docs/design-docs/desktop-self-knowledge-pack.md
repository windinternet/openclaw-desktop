...
- Workbench 事项预览可发起 `work_matter_plan` ActionRun 生成计划草案；Desktop 会带入来源 `workItemPath` / `workItemId`，使用 `work-matter-plan.md` / `buildWorkMatterPlanPrompt` 要求 Agent 读取事项目标、状态、验收标准、关联资料、关联计划、执行记录和关联成果，再提出 `plans/active/` 写入建议。写入或更新计划文件必须通过 `approval_required`，该入口不静默写计划、不自动更新事项、不沉淀成果、不更新知识库或写复盘。
