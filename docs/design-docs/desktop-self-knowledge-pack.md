...
- Teams 自然语言编排和快速创建 Gateway Agent 入口也复用同一共享事项候选；用户可在发起 `agent_team_compose` 或 `gateway_agent_create` ActionRun 前选择已有事项并写入 `workItemPath` / `workItemId`。用户跳过选择时仍进入未归属诊断；该入口不自动创建事项、不猜测归属、不替代 Repository Context 或审批。
- RepositoryGate 的知识库语义映射和工作台语义映射入口也复用同一共享事项候选；用户可在发起 `knowledge_repository_map` 或 `workbench_repository_map` ActionRun 前选择已有事项并写入 `workItemPath` / `workItemId`。该入口只做结构识别和映射保存，不自动创建事项、不猜测归属、不改写仓库内容、不绕过 Repository Context 或审批。
