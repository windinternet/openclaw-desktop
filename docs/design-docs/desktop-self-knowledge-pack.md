...
- 仓库本地 `tool / script / workflow` 已由外部 runner 审批、运行或完成后，Gateway 可调用 `desktop.repository.assets.execution.record`，传入 `repoPath`、`assetId` 或仓库相对 `path`、`status`，以及可选 `runner / command / resultSummary / outputArtifactId / repositoryOutputPath / workItemPath / executedAt`。Desktop 会写入 `runs/assets/YYYYMMDD-HHmmss-<assetId>.md`，维护 `runs/assets/index.md` 作为可扫读运行索引，并刷新 `outputs/assets/index.md` 的 `execution / lastRun / review / reviewResult` 线索；该命令只归档执行事实，不执行资产、不授予权限、不自动写复盘、不更新事项、不沉淀成果、不更新知识库。
