...
- Knowledge 写入审批应携带 `repositoryWrite.writes`，逐项列出 Wiki 条目、`wiki/index.md` 和 `wiki/log.md` 的完整内容；用户批准后 Desktop 通过 `applyKnowledgeRewriteApproval` 写入当前绑定仓库知识库边界内文件，并拒绝 `sources/`、`work/`、`plans/` 等越界路径。该能力不会自动勾选尾动作、更新事项状态、沉淀成果或写复盘。
