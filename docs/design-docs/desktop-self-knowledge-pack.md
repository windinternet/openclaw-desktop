...
- Workbench Snapshot 会把 `runs/assets/index.md` 合并进 Repository runs；Dashboard 会从中读取资产运行结果作为最近成果/本周成果线索，并把 `review: pending` 的资产运行显示为 `asset-run:review-pending` 待确认，跳转 Workbench reviews 时携带 `assetRunPath` 和可选 `workItemPath`。Workbench reviews 会显示“资产运行复盘”卡片，可基于 `runs/assets/*.md` 创建 `reviews/weekly/YYYY-MM-DD-asset-run-*-review.md` 草稿，并把草稿链接回来源事项 `## 复盘`。该入口只观察、路由和写复盘草稿，不执行资产、不授予权限、不更新事项状态、不沉淀成果、不更新知识库。
