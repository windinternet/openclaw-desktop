...
- Repository `outputs/index.md` 是可扫读的 Artifact 目录；条目会暴露 `artifact://` 引用、来源、创建时间、更新时间、预览、格式、摘要、预览计划、预览卡片、内容抽取状态、复用分类和标签，详细审计以单个产物 markdown 为准。带 `reuseKind` 的条目也会进入 `outputs/assets/index.md`，作为当前 Repository 可复用资产目录的第一片。
