...
- Dashboard 复盘类尾动作进入 Workbench reviews 视图时，Desktop 会显示“复盘收尾动作”卡片，保留来源事项、`reviews/weekly/` 目标和 `desktop.artifacts.execution.review.write` 线索；卡片可先创建 `reviews/weekly/YYYY-MM-DD-work-*-tail-action-*-review.md` 事项复盘草稿。草稿只提供核对清单和正文框架，不自动确认复盘、不更新事项、不勾选尾动作。
...
- ActionRun 关联事项后追加的 `## 收尾动作` 会进入 Dashboard 待确认；Dashboard 会把它们分类为 `tail-action:status / output / knowledge / review`，分别导向 Workbench 状态处理、Artifacts、Knowledge 或 Workbench 复盘后续。目标 URL 会携带 `tailAction / tailActionId / workItemPath`；Artifacts 会打开 AI 产物创建入口并带上来源事项，成果类尾动作会预填沉淀成果意图，Workbench/Knowledge 会显示来源上下文，复盘类尾动作可在 Workbench 写入事项复盘草稿。标记完成只写回该条 checklist，不自动执行底层更新。
