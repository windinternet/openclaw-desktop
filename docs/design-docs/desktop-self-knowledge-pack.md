...
- Workbench Snapshot 会把 `runs/assets/index.md` 合并进 Repository runs；Dashboard 会从中读取资产运行结果作为最近成果/本周成果线索，并把 `review: pending` 的资产运行显示为 `asset-run:review-pending` 待确认，跳转 Workbench reviews。该入口只观察和路由，不写复盘、不执行资产、不授予权限、不更新事项状态、不沉淀成果、不更新知识库。
