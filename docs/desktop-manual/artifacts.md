...
已导入的文本、代码和 HTML 文件副本可以进一步记录 `contentExtract`。Gateway 可以调用 `desktop.artifacts.content.extract`，让 Desktop 只读取 Artifact storage 中的导入副本，写入读取字节数、文本长度、是否截断、抽取片段和抽取时间，并在 `repoPath` 就绪时刷新 Repository output。该命令不读取任意本地路径、不解析 Office/PDF/媒体文件、不执行命令，也不授予额外权限。
