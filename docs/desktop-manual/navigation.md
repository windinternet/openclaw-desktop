...
Workbench 面向事项、计划、运行记录、成果和复盘。它对应仓库中的 `work/`、`plans/`、`runs/`、`outputs/` 和 `reviews/`。当 Dashboard 复盘类尾动作进入 Workbench reviews 视图时，Workbench 会显示“复盘收尾动作”卡片，提示用户核对执行记录、成果和知识变更，并把确认后的复盘写入 `reviews/weekly/`；卡片会暴露 `desktop.artifacts.execution.review.write` 命令线索，但不会自动写复盘、更新事项或勾选尾动作。
