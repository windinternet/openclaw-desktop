...
`action-run-knowledge:<runId>` 不是事项 `## 收尾动作` checklist ID，所以 Knowledge 不显示“确认已处理并完成尾动作”，也不会勾选任何事项尾动作。该入口不自动写 Wiki、不更新事项状态、不沉淀成果、不写复盘、不移动事项文件；写入仍必须通过 ActionRun 审批。
